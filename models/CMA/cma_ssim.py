...
class Model(nn.Module):
    def __init__(self, args):
        ...
        #     attn_mask=False
        # ) for i in range(self.conv_eeg_audio_number)])
        self.cma_layer = args.cma_layer if isinstance(self.cm_attn[0], MultiheadAttention) else 1
        ...
        self.proj_images = nn.Conv1d(args.eeg_channel, 16, 1, padding=0, bias=False)
        self.proj_images2 = nn.Conv1d(args.eeg_channel, 16, 1, padding=0, bias=False)
        self.proj_audio = nn.Conv1d(args.audio_channel, 16, 1, bias=False)
        self.proj_audio2 = nn.Conv1d(args.audio_channel, 16, 1, bias=False)
    ...
    def forward(self, bs1, bs2, beeg, targets):
        ...
        for i in range(2):
            ...
            # ssim
            temp1 = ssim(normalization(data[i * 3].unsqueeze(1)), normalization(data[i + 1].unsqueeze(1)), data_range=1, size_average=False)
            data_dot = temp1.unsqueeze(-1) if data_dot is None else torch.cat([data_dot, temp1.unsqueeze(-1)], dim=-1)
