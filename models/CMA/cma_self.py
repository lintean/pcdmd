...
class CrossAttention(nn.Module):
    ...
    def update_qkv(self, query, key, value):
        q = self.embedding_query(query.clone())
        if self.args.q_act:
            q = torch.relu(q)
        k = self.embedding_key(key.clone())
        if self.args.k_act:
            k = torch.relu(k)
        v = self.embedding_value(value.clone())
        if self.args.v_act:
            v = torch.tanh(v)
        return q, k, v
    ...
    def forward(self, query, key, value, **kwargs):
        ...
        if self.args.cmaout:
            ...
            if self.args.cmaout_act:
                attn_value = torch.relu(attn_value)
        return attn_value, attn_weights
...
class Model(nn.Module):
    ...
    def difussion(self, wav):
        wavs = []
        length = self.h - 1
        for i in range(length):
            wavs.append(wav * (i + 1) * (1 / length))
        return torch.concat([wav * 0] + wavs, dim=1)
    ...
    def forward(self, bs1, bs2, beeg, targets):
        ...
        if "encoder" in self.args and self.args.encoder:
            ...
            # wavA = self.difussion(wavA)
            # wavB = self.difussion(wavB)
