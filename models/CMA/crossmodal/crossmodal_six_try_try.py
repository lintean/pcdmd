...
class CNN(nn.Module):
    def __init__(self, embed_dim=30,
        ...
        self.proj_images = nn.Conv1d(10, 30, 1, padding=0, bias=False)
        self.proj_images2 = nn.Conv1d(10, 30, 1, padding=0, bias=False)
        # self.proj_images = nn.Conv2d(1, 1, (1, 4), stride=(1, 4))
        # self.proj_images2 = nn.Conv2d(1, 1, (1, 4), stride=(1, 4))
        self.proj_audio = nn.Conv1d(10, 30, 1, bias=False)
        self.proj_audio2 = nn.Conv1d(10, 30, 1, bias=False)

        self.attention_fcn = nn.Sequential(
            nn.Linear(60, 60), nn.ReLU(), nn.Dropout(p=0),
            nn.Linear(60, 60)
        )
    ...
    def get_network(self, self_type='a2e', layers=-1):
        if self_type in ['a2e']:
            ...
            embed_dim, num_heads, attn_dropout = 30, 5, 0
    ...
    def cross_modal2(self, x_audio, x_eeg, is_images=False, conv1D=False):
        ...
        # eeg→audio
        eeg_trans_audio = self.trans_e2a2(proj_x_eeg, proj_x_audio, proj_x_audio)
    ...
    def forward(self, x):
        ...
        wavA = x[0, 0, 0:1, :].unsqueeze(0).unsqueeze(0)
        eeg  = x[0, 0, 1:65, :].unsqueeze(0).unsqueeze(0)
        wavB = x[0, 0, 65:66, :].unsqueeze(0).unsqueeze(0)

        wavA = self.conv1(wavA).squeeze(0).permute(1, 2, 0)
        eeg = self.conv2(eeg).squeeze(0).permute(1, 2, 0)
        wavB = self.conv3(wavB).squeeze(0).permute(1, 2, 0)
