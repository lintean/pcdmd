...
class CNN(nn.Module):
    def __init__(self, embed_dim=30,
        ...
        self.proj_audio = nn.Conv1d(1, 64, 1, bias=False)
        self.proj_audio2 = nn.Conv1d(1, 64, 1, bias=False)
    ...
    def cross_modal(self, x_audio, x_eeg, is_images=False, conv1D=False):
        ...
        audio_trans_eeg = self.trans_a2e(proj_x_audio, x_eeg, x_eeg)
        # eeg_trans_audio = self.trans_e2a(proj_x_eeg, x_audio, x_audio)

        # temp = audio_trans_eeg.permute(1,2,0)
        # print(temp)

        audio_trans_eeg=audio_trans_eeg.permute(1,0,2)
        # eeg_trans_audio = eeg_trans_audio.permute(1,0,2)

        return audio_trans_eeg
    ...
    def cross_modal2(self, x_audio, x_eeg, is_images=False, conv1D=False):
        ...
        audio_trans_eeg = self.trans_a2e(proj_x_audio, x_eeg, x_eeg)
        # eeg_trans_audio = self.trans_e2a2(proj_x_eeg, x_audio, x_audio)
    ...
    def forward(self, x):
        ...
        wavA_total = wavA.clone()
        wavB_total = wavB.clone()

        # --------------------------------------------------------------------

        for i in range(64):
            eeg_temp  = eeg[0:1, :, i:i+1]
            # print(torch.t(eeg[0, :, i:i+1]))

            wavA_temp = self.cross_modal(wavA, eeg_temp, is_images=False, conv1D=False)
            wavB_temp = self.cross_modal2(wavB, eeg_temp, is_images=False, conv1D=False)

            # print(wavA_temp.shape)
            # print(wavB_temp.shape)

            wavA_total = torch.cat([wavA_total, wavA_temp], dim=2)
            wavB_total = torch.cat([wavB_total, wavB_temp], dim=2)

            # print(wavA_total.shape)
            # print(wavB_total.shape)
