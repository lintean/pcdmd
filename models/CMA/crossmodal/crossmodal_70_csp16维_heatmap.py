...
# names 为这次训练用到的被试数据
label = "crossmodal_70_csp16维_heatmap"
ConType = ["No"]
...
class CNN(nn.Module):
    ...
    def cross_modal(self, x_audio, x_eeg, is_images=False, conv1D=False):
        ...
        return eeg_trans_audio, matrixs, in_proj_weight
    ...
    def forward(self, x):
        ...
        wavA = torch.zeros(wavA.shape[0], wavA.shape[1], 16).to(device) + wavA
        wavB = torch.zeros(wavA.shape[0], wavA.shape[1], 16).to(device) + wavB
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    optimzer, mode='min', factor=0.5, patience=10, verbose=True, threshold=0.0001, threshold_mode='rel', min_lr=0, eps=0.001)
