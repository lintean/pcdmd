...
class CNN(nn.Module):
    def __init__(self, embed_dim=30,
        ...
        self.fc = nn.Sequential(
            nn.Linear(fc_number, fc_number), nn.Sigmoid(),
            nn.Linear(fc_number, conv_output_channel), nn.Sigmoid()
        )

        self.output_fc = nn.Sequential(
            nn.Linear(fc_number, fc_number), nn.Sigmoid(),
            nn.Linear(fc_number, 2), nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # textCNN
        # print(wavA.shape)
        wavA1 = self.conv1(wavA).view(-1, conv_output_channel)
        # print(wavA1.shape)
        wavA2 = self.conv2(wavA).view(-1, conv_output_channel)
        # print(wavA2.shape)
        wavA3 = self.conv3(wavA).view(-1, conv_output_channel)
        # print(wavA3.shape)
        wavA = torch.cat([wavA1, wavA2, wavA3], dim=1)
        wavA = self.fc(wavA)

        eeg1 = self.conv1(eeg).view(-1, conv_output_channel)
        eeg2 = self.conv2(eeg).view(-1, conv_output_channel)
        eeg3 = self.conv3(eeg).view(-1, conv_output_channel)
        eeg = torch.cat([eeg1, eeg2, eeg3], dim=1)
        eeg = self.fc(eeg)

        wavB1 = self.conv1(wavB).view(-1, conv_output_channel)
        wavB2 = self.conv2(wavB).view(-1, conv_output_channel)
        wavB3 = self.conv3(wavB).view(-1, conv_output_channel)
        wavB = torch.cat([wavB1, wavB2, wavB3], dim=1)
        wavB = self.fc(wavB)
