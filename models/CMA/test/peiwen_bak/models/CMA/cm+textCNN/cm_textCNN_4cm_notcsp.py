...
class CNN(nn.Module):
    def __init__(self, embed_dim=30,
        ...
        self.conv_front_eeg = nn.Sequential(
            nn.Conv1d(eeg_channel, conv_eeg_channel, 3),
            nn.ELU(),
        )
        self.conv_front_wavA = nn.Sequential(
            nn.Conv1d(audio_channel, conv_eeg_channel, 3),
            nn.ELU(),
        )
        self.conv_front_wavB = nn.Sequential(
            nn.Conv1d(audio_channel, conv_eeg_channel, 3),
            nn.ELU(),
        )
    ...
    def forward(self, x):
        ...
        eeg = x[0, 0, 1: 1+eeg_channel, :]
        eeg = torch.t(eeg).unsqueeze(0)
        wavB = x[0, 0, 1+eeg_channel: 2+eeg_channel, :]
        wavB = torch.t(wavB).unsqueeze(0)

        eeg = self.conv_front_eeg(eeg.transpose(1, 2))
        wavA = self.conv_front_wavA(wavA.transpose(1, 2))
        wavB = self.conv_front_wavB(wavB.transpose(1, 2))
...
clip = 0.8
optimzer = torch.optim.Adam(myNet.parameters(), lr=0.00001)
