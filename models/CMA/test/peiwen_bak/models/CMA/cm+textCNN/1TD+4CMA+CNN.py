...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.proj_images = nn.Conv1d(self.TimeDelayNum, conv_eeg_channel, 1, padding=0, bias=False)
        self.proj_images2 = nn.Conv1d(self.TimeDelayNum, conv_eeg_channel, 1, padding=0, bias=False)
        ...
        self.td_attn = nn.ModuleList([MultiheadAttention(
    ...
    def select_max(self, wavA, wavB, eeg, origin_wavA, origin_wavB):
        ...
        max_index = torch.max(weight, 1).indices[0]
        # print(max_index)
        # max_index = 0
    ...
    def forward(self, x):
        ...
        for i in range(2):
            data[3*i], weight[3*i] = self.td_attn[3*i](query=kv[3*i], key=data[3*i], value=data[3*i])

        for i in range(conv_eeg_audio_number):
            data[i] = data[i].permute(1, 0, 2)

        # wav Channel 16 to 1
        # wavA, wavB, weightA, weightB, max_index = self.select_max(wavA, wavB, eeg, origin_wavA, origin_wavB)

        # wav Channel 1 to 16
        # data[1] = self.proj_images(data[1])
        # data[2] = self.proj_images2(data[2])
        ...
        # multihead_attention Input shape: Time x Batch x Channel
        # wav and eeg shape: (Batch, Channel, Time)
        data[1] = self.ln(data[1].transpose(1, 2)).transpose(1, 2)
        data[2] = self.ln(data[2].transpose(1, 2)).transpose(1, 2)
        kv = [data[1], data[0], data[3], data[2]]
        weight = [0 for i in range(conv_eeg_audio_number)]
        for i in range(conv_eeg_audio_number):
