...
class CNN(nn.Module):
    def __init__(self, embed_dim=30,
        ...
        self.proj_images = nn.Conv1d(64, 30, 1, padding=0, bias=False)
        self.proj_images2 = nn.Conv1d(64, 30, 1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        wavA, eegA = self.cross_modal(wavA, eeg, is_images=False, conv1D=True)
        wavB, eegB = self.cross_modal2(wavB, eeg, is_images=False, conv1D=True)

        wavA = wavA.transpose(1, 2)
        wavA = wavA.unsqueeze(0)
        eegA = eegA.transpose(1, 2)
        eegA = eegA.unsqueeze(0)
        wavB = wavB.transpose(1, 2)
        wavB = wavB.unsqueeze(0)
        eegB = eegB.transpose(1, 2)
        eegB = eegB.unsqueeze(0)

        wavA = self.conv1(wavA)
        eegA = self.conv2(eegA)
        wavB = self.conv3(wavB)
        eegB = self.conv4(eegB)
        ...
        x = torch.cat([wavA, eegA, wavB, eegB], dim=1)
