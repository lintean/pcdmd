...
# names 为这次训练用到的被试数据
label = "crossmodal_70_csp16维_1卷积"
ConType = ["No"]
...
class CNN(nn.Module):
    ...
    def cross_modal(self, x_audio, x_eeg, is_images=False, conv1D=False):
        ...
        eeg_trans_audio = self.trans_e2a(x_eeg, proj_x_audio, proj_x_audio).permute(1,0,2)
    ...
    def cross_modal2(self, x_audio, x_eeg, is_images=False, conv1D=False):
        ...
        # audio_trans_eeg = self.trans_a2e(proj_x_audio, x_eeg, x_eeg).permute(1,0,2)
        eeg_trans_audio = self.trans_e2a2(x_eeg, proj_x_audio, proj_x_audio).permute(1,0,2)
