...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        weightA = self.TD_fc(weightA)

        wavB, weightB = self.ChannelWav(wavB, eeg)
        weightB = self.TD_fc(weightB)

        # weight = torch.add(weightA,weightB)
        # weightA = weight
        # weightB = weight

        wavA = self.WeightWav(wavA, weightA)
        wavB = self.WeightWav(wavB, weightB)
