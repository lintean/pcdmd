...
class CNN(nn.Module):
    def __init__(self, embed_dim=30,
        ...
        self.TD_fc = nn.Sequential(
            nn.Linear(eegNum, 1),
            # nn.Linear(eegNum,eegNum),nn.ReLU(),
        )
        self.TD_fcA = nn.Sequential(
            nn.Linear(eegNum, 1),
            # nn.Conv2d(1,wavNum,(wavNum,eegNum)),nn.ReLU(),
        )
        self.TD_fcB = nn.Sequential(
            nn.Linear(eegNum, 1),
            # nn.Conv2d(1,wavNum,(wavNum,eegNum)),nn.ReLU(),
        )
    ...
    def ChannelWav(self, wav):
        ...
        for k in range(TimeDelayNum):
            # print(k + 1, end=" ")
            # print(-TimeDelayEnd - abs(TimeDelaySta) + k)
    ...
    def forward(self, x):
        ...
        # print("eeg:", end=" ")
        # print(TimeDelayEnd, end=" ")
        # print(-abs(TimeDelaySta) - 1)

        # Time Delay 机制
        weight = F.adaptive_avg_pool1d(wavA.unsqueeze(0).unsqueeze(0), 1).squeeze(0)
        wavA = self.ChannelWav(wavA)
        wavB = self.ChannelWav(wavB)

        weight = self.weight_fc(weight)
        weight_temp = torch.cat([weight[:, i] for i in range(4) for j in range(4)]).unsqueeze(0)
        weight = torch.zeros(wavA.shape[0], weight_temp.shape[1]).to(device) + weight_temp

        wavA = torch.mul(wavA, weight)
        wavB = torch.mul(wavB, weight)
