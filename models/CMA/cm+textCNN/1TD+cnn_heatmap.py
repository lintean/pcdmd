...
CNN_file = "./CNN_normal_heatmap.py"
...
class CNN(nn.Module):
    ...
    def forward(self, x, need_weight=False):
        ...
        if need_weight:
            return output, weightA, weightB, max_index, wA, wB
        else:
            return output
...
lr = 0.0001
td_params = list(map(id, myNet.td_attn.parameters()))
base_params = filter(lambda p: id(p) not in td_params, myNet.parameters())
params = [{'params': base_params},
          {'params': myNet.td_attn.parameters(), 'lr': lr * 1}]
