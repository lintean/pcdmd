...
# names 为这次训练用到的被试数据
args.label = "KUL_conv_half_combine"
args.ConType = ["No"]
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.eeg_change = nn.ModuleList([
            nn.Conv1d(self.channel_origin[index], self.channel_target[index], 9, padding=4)
    ...
    def forward(self, x):
        ...
        for index in range(len(split)):
            ...
            for i in range(len(split[index])):
                split[index][i] = self.eeg_change[index](split[index][i].transpose(1, 2))
