...
# 所用的数据目录路径
args.data_document_path = device_to_use.origin_data_document + "/DTU_csv_Jon2021B"

# 输入数据选择
# label 为该次训练的标识
# ConType 为选用数据的声学环境，如果ConType = ["No", "Low", "High"]，则将三种声学数据混合在一起后进行训练
# names 为这次训练用到的被试数据
args.label = "4cma_dot_fft_最大池化"
args.ConType = ["No"]

args.CNN_file = "./CNN_normal.py"
args.CNN_split_file = "./CNN_split.py"
args.data_document = "./data/mb_dtu_1"
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.net_eeg_channel_per_band = 4
        ...
        self.eeg_change = nn.ModuleList([
            nn.Linear(args.eeg_channel_per_band, self.net_eeg_channel_per_band
    ...
    def split_bands(self, data):
        temp = []
        channel = int(data.shape[2] / args.eeg_band)
        for i in range(args.eeg_band):
    ...
    def forward(self, x):
        ...
        eeg = torch.cat([eeg[i] for i in range(len(eeg))], dim=2)
        ...
        visualization_weights.append(DotMap(data=wavA, title="wavA before cma"))
        visualization_weights.append(DotMap(data=eeg, title="eeg before cma"))
        visualization_weights.append(DotMap(data=wavB, title="wavB before cma"))
        ...
        visualization_weights.append(DotMap(data=data[0], title="eeg(kv) after cma with wavA"))
        visualization_weights.append(DotMap(data=data[1], title="wavA(kv) after cma with eeg"))
        visualization_weights.append(DotMap(data=data[2], title="wavB(kv) after cma with eeg"))
        visualization_weights.append(DotMap(data=data[3], title="eeg(kv) after cma with wavB"))

        # dot
        # wav and eeg shape: (Batch, Channel, Time)
        data_dot = None
        for i in range(2):
            temp1 = self.time_dot(data[i * 3].squeeze(0), data[i + 1].squeeze(0))
            ...
            # temp1 = self.max_pool(temp1.squeeze(1)[None, None, :]).squeeze(0)
            data_dot = temp1.squeeze(1) if data_dot is None else torch.cat([data_dot, temp1.squeeze(1)], dim=0)
            # data_dot = temp1.squeeze(1) if data_dot is None else torch.abs(data_dot - temp1.squeeze(1))
