...
args.window_length = math.ceil(data_meta.fs * 5)
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, scale_dim):
        super().__init__()
    ...
    def forward(self, query, key):
        ...
        # 编码
        q = self.embedding_q(query)
        k = self.embedding_k(key)

        q = q.contiguous().view(bsz, tgt_len, self.scale_dim)
        k = k.contiguous().view(bsz, -1, self.scale_dim)

        # 相乘
        attn_weights = torch.bmm(q, k.transpose(1, 2))
        attn_weights = attn_weights / ((self.scale_dim ** 0.5)*((k.shape[1] * q.shape[1]) ** 0.25))
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for j in range(2):
            ...
            kv[i] = kv[i].transpose(1, 2)
            weight[i], data[i] = self.cm_attn[i](data[i], kv[i])
