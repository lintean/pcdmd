from ecode.modules_heatmap.transformer import TransformerEncoder
from ecfg import *
...
# 所用的数据目录路径
data_document_path = origin_data_document + "/KUL_band20210124"

CNN_file = "./CNN_normal_heatmap_attend.py"
CNN_split_file = "./CNN_split.py"
data_document = "./data/mb_kul_try_normal"
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.eeg_change = nn.ModuleList([
            nn.Conv1d(64, 8, 1, padding=0, bias=False
        ) for i in range(5)])
    ...
    def forward(self, x, return_=False):
        ...
        eeg = torch.cat([eeg[i] for i in range(len(eeg))], dim=1)
        ...
        for i in range(conv_eeg_audio_number):
            ...
            data[i], weight[i], in_proj_weights, fc_lasts = self.cm_attn[i](data[i], kv[i], kv[i], return_=True)
