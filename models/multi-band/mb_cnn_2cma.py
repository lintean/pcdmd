...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for j in range(self.conv_eeg_audio_number):
            i = j * 3
            data[i] = data[i].permute(2, 0, 1)
            kv[i] = kv[i].permute(2, 0, 1)
            data[i], weight[i] = self.cm_attn[j](data[i], kv[i], kv[i])
        ...
        x = torch.cat([data[i * 3] for i in range(self.conv_eeg_audio_number)], dim=1)
