...
# 所用的数据目录路径
args.data_document_path = device_to_use.origin_data_document + "/KUL_single_multiple3"

# 输入数据选择
# label 为该次训练的标识
# ConType 为选用数据的声学环境，如果ConType = ["No", "Low", "High"]，则将三种声学数据混合在一起后进行训练
# names 为这次训练用到的被试数据
args.label = "KUL_conv_fft_sm"
args.ConType = ["No"]
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.eeg_change = nn.ModuleList([
            nn.ModuleList([
                nn.Conv1d(self.channel_origin[index], self.channel_target[index], 9)
        ...
        self.cm_attn = nn.ModuleList([MultiheadAttention(
            embed_dim=args.window_length - 8,
            num_heads=1,
            # layers=5,
            attn_dropout=0,
            # relu_dropout=0,
    ...
    def forward(self, x):
        ...
        for index in range(len(split)):
            ...
            for i in range(len(split[index])):
                split[index][i] = self.eeg_change[index][i](split[index][i].transpose(1, 2))
            split[index] = torch.cat(split[index], dim=1)
        wavA, eeg, wavB = split
