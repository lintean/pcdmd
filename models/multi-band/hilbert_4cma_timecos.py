...
import torch.fft
from monai.networks.layers.simplelayers import HilbertTransform
...
# names 为这次训练用到的被试数据
args.label = "hilbert_4cma_timecos"
args.ConType = ["No"]
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.output_fc = nn.Sequential(
            nn.Linear(args.window_length * 2, args.window_length), nn.ReLU(),
        ...
        self.hilbert = HilbertTransform(axis=1)
    ...
    def forward(self, x):
        ...
        # 对数据进行hilbert求包络
        wavA = torch.abs(self.hilbert(wavA.squeeze(0).transpose(0, 1))).transpose(0, 1).unsqueeze(0)
        eeg = torch.abs(self.hilbert(eeg.squeeze(0).transpose(0, 1))).transpose(0, 1).unsqueeze(0)
        wavB = torch.abs(self.hilbert(wavB.squeeze(0).transpose(0, 1))).transpose(0, 1).unsqueeze(0)
