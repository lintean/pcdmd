...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(wav_weight.shape[0]):
            origin_wava[:,:,i] = origin_wava[:,:,i] * wav_weight[i]
            origin_wavb[:,:,i] = origin_wavb[:,:,i] * wav_weight[i]
        for i in range(eeg_weight.shape[0]):
            origin_eeg[i] = origin_eeg[i] * eeg_weight[i]
        origin_eeg = torch.cat(origin_eeg, dim=1)

        data_cnn = [origin_wava, origin_eeg, origin_wavb]
        # CNN
        for i in range(3):
            data_cnn[i] = data_cnn[i].transpose(1, 2)
