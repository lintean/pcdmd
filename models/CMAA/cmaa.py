...
class Model(nn.Module):
    def __init__(self, local):
        ...
        self.local = local
        self.window_length = int(local.split_meta.time_len * local.data_meta.eeg_fs)
        self.conv_output_channel = 10
        self.conv_eeg_audio_number = 4

        if not local.model_meta.transformer:
            ...
            self.cm_attn = nn.ModuleList([TransformerEncoder(
                embed_dim=16,
                num_heads=1,
                layers=local.model_meta.cma_layer,
                attn_dropout=0,
                relu_dropout=0,
                res_dropout=0,
                embed_dropout=0,
        ...
        self.cma_layer = local.model_meta.cma_layer if isinstance(self.cm_attn[0], MultiheadAttention) else 1
        ...
        self.conv2 = nn.Conv1d(8, 16, 18, stride=3)

        self.proj_images = nn.Conv1d(local.data_meta.eeg_chan, 16, 1, padding=0, bias=False)
        self.proj_images2 = nn.Conv1d(local.data_meta.eeg_chan, 16, 1, padding=0, bias=False)
        self.proj_audio = nn.Conv1d(local.data_meta.wav_chan, 16, 1, bias=False)
        self.proj_audio2 = nn.Conv1d(local.data_meta.wav_chan, 16, 1, bias=False)

        self.lstm_wavA = nn.LSTM(16, 16, 8)
        self.lstm_wavB = nn.LSTM(16, 16, 8)
        self.lstm_eeg = nn.LSTM(16, 16, 8)

        audio_fs = local.data_meta.wav_fs
    ...
    def forward(self, bs1, bs2, beeg, targets):
        ...
        if "encoder" in self.local.model_meta and self.local.model_meta.encoder:
