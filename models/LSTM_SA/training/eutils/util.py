...
def TsneSimple(x, y):
    ...
    path = "./data/tsne_temp/"
    np.save(makePath(path) + "embedding_train", embedding_train)
...
def heatmap(_, log_path=None, window_index=None, epoch=None, name=None):
    a = _.data
    ...
    heatmap = sns.heatmap(pd.DataFrame(np.round(a, 2)), xticklabels=False, yticklabels=False, square=True, cbar=False)
    ...
    if log_path is not None:
        save_path = makePath(log_path + "/picture/" + name + "/window_index_" + str(window_index) + "/")
        fig.savefig(save_path + "epoch_" + str(epoch) + " " + title + ".png", format='png', transparent=True)

        if need_save:
            pd.DataFrame(a).to_csv(save_path + "epoch_" + str(epoch) + " " + title + ".csv", header=False, index=False)
    plt.close(fig)
...
def get_gpu_with_max_memory(gpu_list):
...
def save_model(local):
    """
    保存模型
    @param local: subject参数
    """
    torch.save({'epoch': local.epoch, 'state_dict': local.model.state_dict(),
                'optimizer': local.optimizer.state_dict(), 'alpha': local.loss.alpha, 'gamma': local.loss.gamma},
...
def load_model(local):
    ...
    加载模型
    @param local: subject参数
    @return: local : subject参数
    """
    model_CKPT = torch.load(f"{makePath(local.log_path)}/model-{local.name}.pth.tar")
    local.model.load_state_dict(model_CKPT['state_dict'])
    local.optimizer.load_state_dict(model_CKPT['optimizer'])
    return local