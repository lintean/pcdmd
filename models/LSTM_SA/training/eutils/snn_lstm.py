...
class SLSTM(nn.LSTM):
    ...
    def __init__(self, vth: float = 0.2, snn_process: bool = True, *args, **kwargs):
        ...
        # snn
        self.forget_gate_s = slayers.ModuleLayer(self.forget_gate, snn_process=self.snn_process)
        self.input_gate_s = slayers.ModuleLayer(self.input_gate, snn_process=self.snn_process)
        self.output_gate_s = slayers.ModuleLayer(self.output_gate, snn_process=self.snn_process)
        self.cell_gate_s = slayers.ModuleLayer(self.cell_gate, snn_process=self.snn_process)
        self.output_linear_s = slayers.ModuleLayer(self.output_linear, snn_process=self.snn_process)
        self.spike = slayers.LIFSpike(snn_process=self.snn_process, vth=self.vth)
