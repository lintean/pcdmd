...
import torch
from eutils.lsnn import LSNNRecurrent
from eutils.lsnn import LSNNParameters
from eutils.snn_encoder import BSA_Encoder
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.classify = nn.Linear(self.lstm_hidden, self.input_size)
        self.ann_decode = nn.Linear(self.input_size, self.output_size)
        ...
        self.BNl = slayers.TdBatchNorm1d(self.input_size, sp=self.sp)
        ...
        self.classify_s = slayers.ModuleLayer(self.classify, sp=self.sp, bn=self.BNl)
        # self.classify_s = LILinearCell(self.lstm_hidden, self.output_size)
    ...
    def dot_product(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        attn_weights = attn_weights / q.shape[1]
    ...
    def forward(self, x, targets):
        ...
        # ENCODE
        eeg, origin = self.bsa.encode(eeg)
