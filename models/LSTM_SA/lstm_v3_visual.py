...
import norse.torch.functional.lif
...
class Model(nn.Module):
    ...
    def forward(self, bs1, bs2, beeg, targets):
        ...
        self.visualization_weights.append(
            DotMap(
                data=eeg[0].clone(),
                title="encoder",
                need_save=True,
                figsize=[eeg.shape[2], eeg.shape[1]]
            )
        )
        ...
        if self.need_sa:
            ...
            self.visualization_weights.append(
                DotMap(
                    ...
                    title="saweight",
                    need_save=True,
                    figsize=[weight.shape[2], weight.shape[1]]
                )
            )
        ...
        self.visualization_weights.append(
            DotMap(
                data=lstm_output[0].clone(),
                title="rsnn",
                need_save=True,
                figsize=[lstm_output.shape[2], lstm_output.shape[1]]
            )
        )
        ...
        return output, targets, self.visualization_weights
