...
def get_data_loader(local: DotMap = DotMap(), args: DotMap = DotMap(), **kwargs):
    ...
    tng_set = CurrSet(eeg=eeg, wav1=wav1, wav2=wav2, window=train_window, window_meta=window_metadata, args=args, dev=dev)
    tes_set = CurrSet(eeg=eeg, wav1=wav1, wav2=wav2, window=test_window, window_meta=window_metadata, args=args, dev=dev)
    ...
    with torch.set_grad_enabled(mode == "train"):
        ...
        for (wav1, wav2, eeg, label, batch_idx) in dataloader:
            ...
            if mode == "train":
                ...
                if local.aad_model.warmup is not None:
                    with local.aad_model.warmup.dampening():
                        pass
...
def trainEpoch(data_loader, local, args):
    ...
    for epoch in range(args.max_epoch):
        ...
        # 学习率衰减
        if local.aad_model.sched is not None:
            if local.aad_model.warmup is not None:
                with local.aad_model.warmup.dampening():
        ...
        current_lr = local.aad_model.optim.state_dict()['param_groups'][0]['lr']
...
def load(data_loader: tuple, local: DotMap = DotMap(), args: DotMap = DotMap()):
    model_file = f"{args.load_model_path}/flod_{local.current_flod}/model-{local.name}.pth.tar"
