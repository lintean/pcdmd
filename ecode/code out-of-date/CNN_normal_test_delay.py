...
def add_delay(data):
    ...
    EEG = data.iloc[:, 1:channel_number-1]
...
def train(train): 
    ...
    for turn in range(math.floor(train.shape[0] / batch_size)):
        ...
        batchData = np.ndarray((0,1,temp.shape[0], window_length - delay))
        allTarget = []
        for k in range(batch_size):
            input = train[turn*batch_size + k]["data"]
            input = add_delay(input).T
