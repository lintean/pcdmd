...
def getSEData(test_data):
    ...
    for turn in range(math.floor(test_data.shape[0] / batch_size)):
        ...
        x = torch.tensor(batchData, dtype=torch.float32)
        x = x.to(device)
        SE_data, weight = myNet(x, type="SEoutput")

        data_list.append(SE_data.cpu().detach().numpy())
        data_target.append(allTarget[0])
        data_weight.append(weight.cpu().detach().numpy())

    data = []
    for i in range(len(data_list)):
        d = dict()
        d["data"] = data_list[i]
        d["direction"] = data_target[i] + 1
        d["weight"] = data_weight[i]
        d["index"] = test_data[i]["index"]
        data.append(d)
...
def main(name = "S2", data_document="./data_new"):
    ...
    # 保存SE数据
    SE_train = getSEData(data[0])
    SE_test = getSEData(test_data)

    data_pf = [SE_train, SE_test]
    data_pf = np.array(data_pf)

    SE_path = "./SE_data/"
    np.save(util.makePath(SE_path) + name, data_pf)
