...
def train(train, epoch):
    ...
    for turn in range(math.floor(train.shape[0] / batch_size)):
        ...
        x = x.to(device)
        out, weightA, weightB = myNet(x)
        weightAs = weightA if turn == 0 else torch.add(weightAs, 1, weightA)
        weightBs = weightB if turn == 0 else torch.add(weightBs, 1, weightB)
        loss = loss_func(out, torch.tensor(allTarget, dtype=torch.long).to(device))
        losses = losses + loss.cpu().detach().numpy()
        loss.backward()
        optimzer.step()

    ma1 = pd.DataFrame(weightAs.cpu().detach().numpy())
    ma2 = pd.DataFrame(weightBs.cpu().detach().numpy())
    ma1.to_csv("./picture/" + names[0] + "/" + str(epoch) + "_weightA_avg.csv")
    ma2.to_csv("./picture/" + names[0] + "/" + str(epoch) + "_weightB_avg.csv")
