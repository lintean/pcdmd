...
train_dataset = read('rwcp_classify_train_data.pkl')

train_x = torch.tensor(train_dataset[0]).float()
train_y = torch.tensor(train_dataset[1]).long()
train_dataset = [train_x,train_y]

test_dataset = read('rwcp_classify_test_data.pkl')
...
optimizer =torch.optim.Adamax(snn.parameters(),lr=learning_rate,betas=(0.9,0.999))
...
for epoch in range(num_epochs):
        ...
        if (epoch+1)==num_epochs:
            torch.save(snn.state_dict(), 'rwcp_DNN.pth' )
