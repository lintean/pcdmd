...
V1_feature_maps=[4,20,100]
V1_thres=[10,15,15]
V1_pools=[2,2,6]#2,2,4
    ...
    if m >= C_l:
        ...
        for epoch in range(1, max_epoches + 1):
            ...
            for (images, labels) in train_loader:
                ...
                print('V1 并行 0 层,epoch ' + str(epoch) + ', 第', str(i), '次训练loss = ', str(m))
                ...
                if epoch > 1:
                    if m < C_l:
                        # 说明收敛
                        print('V1 并行 0 层,epoch ' + str(epoch) + ', 第', str(i), '次训练  '+'收敛，停止运行')
    ...
    if m >= C_l:
        ...
        for epoch in range (1, max_epoches + 1):
            ...
            for (images, labels) in train_loader:
                ...
                print ('V1 并行 1 层,epoch ' + str (epoch) + ', 第', str (i), '次训练loss = ', str (m))
                if m < 0:
                    print ('学习率过大，请调小')
                    exit (0)
                if epoch > 1:
                    if m < C_l:
                        # 说明收敛
                        print ('V1 并行 1 层,epoch ' + str (epoch) + ', 第', str (i),
                               '次训练  ' + '收敛，停止运行')
                        write_pkl (os.path.join (weight_save_root_path, W2_name), W2)
                        write_pkl ('TIDIGITS_chain_loss_record1.pkl', chain_loss_record1)
                        write_pkl ('TIDIGITS_chain_loss_record2.pkl', chain_loss_record2)
                        print ('finish !!!!!')
                        return
