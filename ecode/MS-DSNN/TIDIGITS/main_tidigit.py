...
# from MSSNN import *
from tidigit_SpikeClassifier import*
...
# 根据ANN 初始化参数
ANN_dict = torch.load('tidigit_DNN.pth','cuda:0')
...
#pretrained_dict = {k.replace('.','_').replace('.0.' ,'_') : v for k, v in ANN_dict.items() if k.replace('.','_').replace('.0.' ,'_') in SNN_dict}
...
#optimizer = torch.optim.SGD(snn.parameters(), lr=learning_rate,momentum=0)

optimizer =torch.optim.Adam([
                {'params': snn.fc1_weight,'lr':0.1*learning_rate},
                {'params': snn.fc2_weight, 'lr': learning_rate}
            ],lr=learning_rate)
...
for epoch in range(num_epochs):
        ...
        for i,(images, labels) in enumerate(train_loader):
            ...
            batch = labels.size(0)  
            labels_ = torch.zeros(batch, 11).scatter_(1, labels.view(-1, 1), 1)
            outputs = snn(images)
