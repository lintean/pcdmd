...
dt=.02
C_l=0.0005
spike_thres = [0.1,0.3,0.5]
V1_kernel_sizes=[5,7,9]
V1_feature_maps=[4,30,100]
V1_thres=[10,20,15]
V1_pools=[2,4,6]#2,2,4
# V2_feature_maps=[30,70]
# V2_thres=[15,30]
# V2_kernel_sizes=[5,5]
# V2_scale = [1,0.5,0.25]
...
#     if not os.path.exists(os.path.join(weight_save_root_path, W1_name)):
#         W1 = np.random.normal(0.8, 0.05, [V2_feature_maps[0], 2, V2_kernel_sizes[0], V2_kernel_sizes[0]])
#     else:
#         W1 = read(os.path.join(weight_save_root_path, W1_name))
#     m = model.measure(W1)
#     if m >= C_l:
...
#                 input = images[0][0].numpy()
#                 i += 1
#                 input = cv2.resize(input, (0, 0), fx=V2_scale[scale_id], fy=V2_scale[scale_id],
...
#     #     W2 = np.random.normal(0.8, 0.05,[V2_feature_maps[1], V2_feature_maps[0], V2_kernel_sizes[1], V2_kernel_sizes[1]])
#     # else:
...
class SCNN_Extractor_V1(object):
    ...
    def extractor(self,train_loader,test_loader):
        ...
        print(len(train_x))
        print(len(test_x))
        write_pkl('tidigit_classify_train_data.pkl',[train_x,train_y])
        write_pkl('tidigit_classify_test_data.pkl',[test_x,test_y])
...
#                 conv_time1, conv_V1 = model.spike_conv(temporal_coding, W1, V2_thres[0], Tmax, dt)
#                 if scale_id == 0:
#                     pool_time1 = model.spike_pooling(conv_time1, 6, 6)
#                 else:
#                      pool_time1 = model.spike_pooling(conv_time1,2,2)
#                 scale_feauture= pool_time1.reshape(-1)
#                 feature_tmp.append(scale_feauture)
