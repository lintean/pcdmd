...
train_dataset = read('tidigit_classify_train_data.pkl')
train_x = torch.tensor(train_dataset[0]).float()
train_y = torch.tensor(train_dataset[1]).long()
train_dataset = [train_x,train_y]

test_dataset = read('tidigit_classify_test_data.pkl')
...
class DNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(1936,240),#1024
            nn.Dropout(0.6),#0.5
            #nn.BatchNorm1d(600),
            nn.ReLU()
        )
      
        self.fc2 = nn.Linear (240, 11) #1024
    def forward(self, x):
...
criterion = nn.CrossEntropyLoss()#loss_fun.apply
#optimizer = torch.optim.(snn.parameters(), lr=learning_rate)
optimizer =torch.optim.Adam(snn.parameters(),lr=learning_rate)
#scheduler = MultiStepLR(optimizer, milestones=[3,], gamma=0.5, last_epoch=-1)
...
for epoch in range(num_epochs):
        ...
        print('Total %d epochs ,learing rate %f ,Train Accuracy of the model of %d Epoch on the 60000 train images: %f' % (num_epochs,learning_rate,epoch+1,100. * float(train_correct) / float(train_total)))
        ...
        print('Total %d epochs ,learing rate %f ,Test Accuracy of the model of %d Epoch on the 10000 test images: %f' % (num_epochs,learning_rate,epoch+1,100 * correct / total))
        acc = 100. * float(correct) / float(total)
        end_time = time.clock()
        print('Time consuming of %d Epoch is : %f\n'%(epoch+1,end_time-start_time))
        acc_record.append(acc)
        weight = snn.state_dict()
        if best_acc<acc:
            best_acc=acc
            best_weight = copy.deepcopy(weight)
        #if (epoch+1)==num_epochs:
        #    torch.save(snn.state_dict(), 'tidigit_DNN.pth' )
           
print('In learing rate %f  , the highest accuracy of training : %f' % (learning_rate,  max(train_acc)))
print('In learing rate %f  , the highest accuracy of testing :%f '%(learning_rate, max(acc_record)))
torch.save(best_weight, 'tidigit_DNN.pth' )
