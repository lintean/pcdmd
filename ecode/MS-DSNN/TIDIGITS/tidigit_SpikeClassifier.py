...
loss_thres=0.08#0.08#0.07
# 常调试参数
batch_size = 32 # 60
learning_rate = 1e-3#1e-3 ###Adam :5e-3
num_epochs = 2000  # max epoch
sigma = 0.2 #0.14##0.1  # 0.1 0.05
# sigmas =[0.04,0.06,0.08,0.1,0.12,0.14,0.16]
#  不常调试参数
thresh=[1,1]#thresh = 1  # neuronal threshold
num_classes = 11
dt = .02
Tmax = 10#10 #6
encodemax = 3
...
def seek_spike(voltage_post,thresh):
...
class Linear_Spike(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ti, W,thresh):  # dropout 用于全连接层
            # 计算膜电压
...
class SpikeMLP(nn.Module):
    def __init__(self):
        ...
        self.fc1_weight = nn.Parameter(torch.tensor(np.zeros([cfg_fc[1], cfg_fc[0]])).float(),
                                       requires_grad=True)
        self.fc2_weight = nn.Parameter(torch.tensor(np.zeros([cfg_fc[2], cfg_fc[1]])).float(),
                                       requires_grad=True)
        # 1.
    ...
    def forward(self, input):
        ...
        ti = input
        h1_spike = self.linear_spike(ti, self.fc1_weight,thresh[0])
        # h1_spike = dropout(h1_spike, drop_rate, self.training)
        h2_spike = self.linear_spike(h1_spike, self.fc2_weight,thresh[1])
        #print('h1',h1_spike)
        #print('h2',h2_spike)
        #exit(0)
        outputs = h2_spike
