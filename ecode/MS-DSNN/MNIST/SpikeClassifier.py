...
loss_thres=0.07#0.07
# 常调试参数
batch_size = 100 # 60
learning_rate = 5e-3 # Adam :5e-3
num_epochs = 200  # max epoch
...
class SpikeMLP(nn.Module):
    def __init__(self):
        ...
        self.fc1_weight = nn.Parameter(torch.tensor(np.random.normal(0.01*interval, 0.01*interval, [cfg_fc[1], cfg_fc[0]])).float(),
                                       requires_grad=True)
        self.fc2_weight = nn.Parameter(torch.tensor(np.random.normal(0.001*interval, 0.001*interval, [cfg_fc[2], cfg_fc[1]])).float(),
                                       requires_grad=True)
        # 1.
