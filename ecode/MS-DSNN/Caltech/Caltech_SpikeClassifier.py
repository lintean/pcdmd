...
loss_thres=0.09#0.07
# 常调试参数
batch_size = 100 # 60
learning_rate = 2e-4 # Adam :5e-3
num_epochs = 500  # max epoch
sigma = 0.2#0.2 #0.14##0.1  # 0.1 0.05
...
class SpikeMLP(nn.Module):
    def __init__(self):
        ...
        self.fc1_weight = nn.Parameter(torch.tensor(np.random.normal(0.01, 0.01, [cfg_fc[1], cfg_fc[0]])).float(),
                                       requires_grad=True)
        self.fc2_weight = nn.Parameter(torch.tensor(np.random.normal(0.001, 0.001, [cfg_fc[2], cfg_fc[1]])).float(),
                                       requires_grad=True)
        # 1.
